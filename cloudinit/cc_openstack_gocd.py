...
def handle(name, cfg, cloud, log, _args):
    try:
        # Ga Agent wrappen conf
        go_agent_wrapper = "/usr/share/go-agent/wrapper-config/wrapper-properties.conf"
        ...
        for key in md['meta']:
            if key.startswith(go_agent_prefix):
                ...
                replace_config(go_server_config, key[len(
                    go_server_prefix):], md['meta'][key])
